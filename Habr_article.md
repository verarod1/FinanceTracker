...
Сейчас трекер полностью закрывает базовые потребности в быстром учете, но архитектура изначально закладывалась так, чтобы слой `FinanceManager` можно было безболезненно расширять. В будущих версиях планирую добавить:

* **Миграция на базу данных.** Чтение из `data.txt` работает отлично на небольших объемах, но для сложной выборки и надежности напрашивается интеграция с SQLite. Замена хранилища не затронет UI-слой.
* **Импорт банковских выписок.** Самая интересная фича - автоматизация ввода. Хочется брать данные о транзакциях из банковских отчетов (Тбанк, Сбер и другие)
* **Экспорт в Excel.** Возможность выгрузить всю статистику в условный `.csv` или `.xlsx` для резервного копирования или построения сложных сводных таблиц на стороне.
...
### Проект полностью открыт и доступен для изучения. 
Ознакомиться с полным исходным кодом, структурой файлов и протестировать сборку локально можно в моем репозитории на GitHub:
[Исходный код FinanceTracker](https://github.com/verarod1/FinanceTracker)
